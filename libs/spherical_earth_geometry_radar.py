...
def incidence_azimuth_to_gcs(incidence_mesh, azimuth_mesh, lambda_c, v_s, h=500e3, re=6371e3, c=299792458.0):
    """
    converts incidence angle - azimuth distance coordinates to xyz GCS coordinates for vectors
    :param incidence_mesh: incidence angle
    :param azimuth_mesh:  azimuth distance on the closeast approach incidence angle circle (spherical earth)
    :param lambda_c: wavelength
    :param v_s: satellite orbital speed (circular orbit)
    ...
    :return: x, y, z
    """
    x = np.zeros_like(incidence_mesh)
    y = np.zeros_like(incidence_mesh)
    z = np.zeros_like(incidence_mesh)

    for ii in range(len(incidence_mesh)):
        # closest approach slant range
        R0_mesh = re * (np.sqrt(cos(incidence_mesh[ii]) ** 2 + 2 * h / re + h ** 2 / re ** 2) - cos(incidence_mesh[ii]))
        # cosine of earth-centric elevation angle of azimuth circle parallel to the orbital plane on the sphere
        cos_theta_e = (re + R0_mesh * cos(incidence_mesh[ii])) / (re + h)
        # elevation coordinate of point
        theta_e = np.arccos(cos_theta_e) * np.sign(incidence_mesh[ii])  # to consider also incidence angles behind nadir
        # azimuth angle coordinate
        theta_a = azimuth_mesh[ii] / (re * cos_theta_e)
        # x coordinate mesh
        x[ii] = re * cos_theta_e * sin(theta_a)
        # y coordinate mesh
        y[ii] = - re * sin(theta_e)
        # z coordinate mesh
        z[ii] = -re + re * cos_theta_e * cos(theta_a)
    return x, y, z
